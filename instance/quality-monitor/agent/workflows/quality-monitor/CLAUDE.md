...
3. **Read `anti-patterns.yaml`** from the workflow directory - this contains:
   ...
   - **ESLint rule recommendations** for preventive measures
4. **Read sample test files** from the listed repositories
5. **Analyze for anti-patterns** using contextual understanding:
   - Review test code in context (understand WHY the code exists)
   - Compare patterns against examples in anti-patterns.yaml
   - Focus on HIGH severity first (hard-coded sleeps, missing assertions, error swallowing)
   - Identify whether the pattern is justified or problematic
   - Note specific file:line locations with surrounding context
...
{if any findings have applicable eslint_rules}
h4. Recommended ESLint Rules

Configure these rules in `.eslintrc.js` to catch issues automatically:

{for each unique eslint rule across findings}
*{rule.name}*
{code:javascript}
{rule.config}
{code}
...
h4. ESLint Adoption Steps

# Install required plugins if needed (e.g., `npm install -D eslint-plugin-jest eslint-plugin-playwright`)
# Add the rules above to your `.eslintrc.js` configuration
# Run `npm run lint` to identify existing violations
# Fix violations or add `eslint-disable` comments for justified exceptions
# Enable linting in CI pipeline to prevent future violations

{endif}

h3. Remediation Priority

# Address all HIGH severity issues first (hard-coded sleeps, missing assertions, error swallowing)
# Create tech debt tickets for MEDIUM issues (disabled tests, duplicated auth)
